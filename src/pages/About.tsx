  ...
  <div className="flex-1 p-4 sm:p-5 bg-secondary flex flex-col gap-4 sm:gap-5">
    <div className="flex items-start gap-4 sm:gap-6 md:gap-10">
      ...
      <div className="flex flex-col gap-2 sm:gap-3">
        <h3 className="text-primary text-lg sm:text-xl lg:text-2xl font-black uppercase leading-6 sm:leading-8">{title}</h3>
        <p className="text-foreground text-base sm:text-lg lg:text-xl font-medium leading-6 sm:leading-7">{description}</p>
      </div>
    </div>
  </div>
);
      ...
      <div className="max-w-[1200px] mx-auto flex flex-col gap-6 sm:gap-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-1">
          {partnerLogos.map((logo, i) => (
            <div key={i} className="h-32 sm:h-40 md:h-48 bg-background flex items-center justify-center p-4 sm:p-6">
              <img src={logo} alt={`Partner ${i + 1}`} className="max-h-24 sm:max-h-28 md:max-h-36 max-w-full object-contain" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 sm:bottom-auto sm:left-auto sm:top-1/2 sm:-translate-y-1/2 sm:right-6 md:right-12 lg:right-24 w-full sm:w-[90%] md:w-[500px] lg:w-[598px] p-4 sm:p-5 bg-primary flex flex-col gap-3 sm:gap-5 z-10">
        ...
        <div className="flex flex-wrap justify-between items-start sm:items-center gap-2 sm:gap-4">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <img src={checkInCircleIcon} alt="" className="w-4 h-4 sm:w-6 sm:h-6 [filter:invert(43%)_sepia(98%)_saturate(1856%)_hue-rotate(196deg)_brightness(97%)_contrast(101%)]" />
            <span className="text-primary-foreground text-xs sm:text-base font-bold leading-5 sm:leading-6">{t("about.map.tag1")}</span>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <img src={checkInCircleIcon} alt="" className="w-4 h-4 sm:w-6 sm:h-6 [filter:invert(43%)_sepia(98%)_saturate(1856%)_hue-rotate(196deg)_brightness(97%)_contrast(101%)]" />
            <span className="text-primary-foreground text-xs sm:text-base font-bold leading-5 sm:leading-6">{t("about.map.tag2")}</span>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <img src={checkInCircleIcon} alt="" className="w-4 h-4 sm:w-6 sm:h-6 [filter:invert(43%)_sepia(98%)_saturate(1856%)_hue-rotate(196deg)_brightness(97%)_contrast(101%)]" />
            <span className="text-primary-foreground text-xs sm:text-base font-bold leading-5 sm:leading-6">{t("about.map.tag3")}</span>
          </div>
        </div>
        <div className="h-px bg-primary-foreground/40" />
        <div className="flex justify-between items-center">
          ...
          <span className="text-primary-foreground text-xs sm:text-base font-medium leading-5 sm:leading-6">{t("about.map.capacity")}</span>
        </div>
      </div>
    </section>
  );
...
const FAQSection = () => {
  ...
  return (
    <section className="px-4 sm:px-6 lg:px-28 py-10 sm:py-16 lg:py-20 bg-secondary">
      <div className="max-w-[1200px] mx-auto flex flex-col gap-6 sm:gap-10">
        <h2 className="text-primary text-2xl sm:text-3xl lg:text-5xl font-black uppercase leading-tight">{t("about.faq.title")}</h2>
        <div className="flex flex-col gap-1">
          {items.map((item, i) => (
            <div
              key={i}
            ...
            >
              <div className="flex items-center justify-between gap-4 sm:gap-10">
                <h3 className="text-primary text-base sm:text-xl lg:text-2xl font-black leading-6 sm:leading-8">{item.question}</h3>
              ...
              {openIndex === i && (
                <p className="text-foreground text-sm sm:text-base font-medium leading-5 sm:leading-6 max-w-[880px]">{item.answer}</p>
              )}
            </div>
          ))}
        </div>
        <p className="text-foreground text-sm sm:text-base font-medium leading-5 sm:leading-6">{t("about.faq.footer")}</p>
      </div>
    </section>
  );
};
  ...
  return (
    <section className="relative px-4 sm:px-6 lg:px-28 py-8 sm:py-10 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto flex flex-col gap-2">
        <p className="text-primary-foreground text-base sm:text-xl font-bold leading-6 sm:leading-7">{t("about.questions.subtitle")}</p>

...
const ServiceCard = ({ icon, title, description, cta = "CTA" }: ServiceCardProps) => (
  <div className="flex-1 min-w-0 p-4 sm:p-5 bg-background border-2 border-secondary flex flex-col justify-between gap-4 sm:gap-5">
    <div className="flex flex-col gap-4 sm:gap-6">
      <div className="w-14 h-14 sm:w-20 sm:h-20 bg-secondary flex items-center justify-center">
        {icon}
      </div>
      <div className="flex flex-col gap-2 sm:gap-3">
        <h3 className="text-primary text-lg sm:text-xl lg:text-2xl font-black uppercase leading-tight">
          {title}
        </h3>
...
const ServicesSection = () => {
  ...
  const services = [
    {
      icon: (
        <img
          src={mapPinSimpleAreaIcon}
          alt="Šķirošanas laukums"
    ...
    },
    {
      icon: (
        <img
          src={lightningIcon}
          alt="Baltās tehnikas pieņemšana"
          className="w-8 h-8 sm:w-10 sm:h-10"
        />
      ),
  ...
  const services2 = [
    {
      icon: (
        <img src={bulldozerIcon} alt="Ēku un būvju demontāža" className="w-8 h-8 sm:w-10 sm:h-10" />
      ),
    ...
    {
      icon: (
        <img src={certificateIcon} alt="Zaļo ēku sertifikāti" className="w-8 h-8 sm:w-10 sm:h-10" />
      ),
